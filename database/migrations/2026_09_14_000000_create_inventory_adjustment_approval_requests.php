...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasTable('inventory_adjustment_request_items')) {
            Schema::create('inventory_adjustment_request_items', function (Blueprint $table): void {
                ...
                $table->foreign('inventory_adjustment_request_id', 'inv_adj_items_request_fk')
                    ->references('id')
                    ->on('inventory_adjustment_requests')
                    ->cascadeOnDelete();
                $table->foreignId('variant_id');
                $table->foreign('variant_id', 'inv_adj_items_variant_fk')
                    ->references('id')
                    ->on('variants')
                    ->restrictOnDelete();
