...
- **Remove** opens a Filament confirmation modal with three choices: detach from this preview layout, permanently delete only the vibe card, or delete the vibe card **and its linked Shopify collection**. All choices are staged until **Push Changes**. The review names collections and their URLs explicitly. Collection deletion removes the collection page, while **every product stays in Shopify, in the local catalogue and in its other collections**. Images are kept. The collection option calls only `collectionDelete` (using `write_products`), never `productDelete`, and drops pending membership edits for that collection. Confirmed deletion removes the collection's local catalogue entries and its own saved vibe draft. Retries tolerate an already-deleted collection.
- Permanent removal deletes only a `shop_your_vibe_card_preview` entry, using the existing `write_metaobjects` scope. Before deletion the worker verifies its fields and checks Shopify reference metadata; shared cards are blocked. It detaches the current preview reference before deleting, rechecks for other references, and resumes safely if Shopify's deletion response is lost. An entry created during an interrupted push can be recovered by its persisted UUID handle. Discarding an unpushed removal cancels that deletion.
