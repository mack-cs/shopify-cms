...
The report layout is 33 columns (`A:AG`). The Shopify inventory quantities shown in Sheets are grouped as `Available` in column G, `Reserved` in H, and `On Hand` in I. `Committed` remains stored in the CMS but is not published to this report. `cms_movement_classification` is column J, `Ignore` is K, and `Action Required` is AE. Columns O:R show the two earliest complete pending orders. Columns S:T project the stock immediately before the second ETA and flag whether stock from the first delivery covers demand between the two ETAs. `Replenishment Date` uses the earliest ETA and `Stock Gap Status` compares it with predicted runout. `Last Updated` remains the final column. Existing workbook layouts are backed up and upgraded by header name before synchronization, so manually sorted rows remain matched by SKU.
