...
The report layout is 34 columns (`A:AH`). The Shopify inventory quantities are grouped together as `Available` in column G, `Committed` in H, `Reserved` in I, and `On Hand` in J. `cms_movement_classification` is column K, `Ignore` is L, and `Action Required` is AD. Columns P:S show the two earliest complete pending orders. `Replenishment Date` uses the earliest ETA and `Stock Gap Status` compares it with predicted runout. `Last Updated` remains the final column. Existing workbook layouts are backed up and upgraded by header name before synchronization, so manually sorted rows remain matched by SKU.
