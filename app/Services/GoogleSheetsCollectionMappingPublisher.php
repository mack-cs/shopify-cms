...
final class GoogleSheetsCollectionMappingPublisher
{
    ...
    {
        return (bool) config('google_sheets.collection_mapping_enabled');
    }
    ...
    public function publish(ShopifyCollectionProductReportRun $run, array $collectionHandles): int
    {
        ...
        $spreadsheetId = trim((string) config('google_sheets.collection_mapping_spreadsheet_id'));
        if ($spreadsheetId === '') {
        ...
        return Cache::lock(
            ...
            max(30, (int) config('google_sheets.collection_mapping_lock_seconds', 300)),
    ...
    private function request(): \Illuminate\Http\Client\PendingRequest
    {
        return Http::withToken($this->accessToken())
            ->acceptJson()
            ->asJson()
            ->timeout(max(5, (int) config('google_sheets.collection_mapping_timeout_seconds', 30)));
    }
