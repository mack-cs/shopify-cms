...
class ShopifyInventoryAdjustmentService
{
    ...
    public function increaseAvailable(Variant $variant, int $quantity, string $referenceUri): void
    {
        $inventoryItemId = trim((string) $variant->shopify_inventory_item_id);
        if ($inventoryItemId === '') throw new \RuntimeException("Variant {$variant->id} has no Shopify inventory item ID.");
        $locationId = trim((string) config('services.shopify.inventory_location_id'));
        if ($locationId === '') {
            $locations = $this->client->graphql('query ProcurementLocation { locations(first: 1) { nodes { id name } } }');
            $locationId = trim((string) data_get($locations, 'locations.nodes.0.id'));
        }
        if ($locationId === '') throw new \RuntimeException('No Shopify inventory location could be resolved.');
